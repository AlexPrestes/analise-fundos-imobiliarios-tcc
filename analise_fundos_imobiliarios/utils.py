...
def aumento_patrimonial_fundo(ds, cnpj_fundo):
    da = ds.sel(CNPJ_Fundo=cnpj_fundo)['Valor_Patrimonial_Cotas'].dropna(
        'Data_Referencia'
    )
    da = da / da.shift(Data_Referencia=1)
    da = da * (
        1
        - ds.sel(CNPJ_Fundo=cnpj_fundo)[
            'Percentual_Amortizacao_Cotas_Mes'
        ].shift(Data_Referencia=1)
    )
    da = np.round(da, 2)
    da[da == np.inf] = np.nan
    da[da == 0.0] = np.nan
    ...
    dc = ds.sel(CNPJ_Fundo=cnpj_fundo)['Cotas_Emitidas'].dropna(
        'Data_Referencia'
    )
    ...
    dp = ds.sel(CNPJ_Fundo=cnpj_fundo)['Patrimonio_Liquido'].dropna(
        'Data_Referencia'
    )
    dk = dp.copy()
    ...
    for n, i, j, k in zip(
        range(len(dc.values)), dc.values, da.values, dp.values
    ):
        da_result[n] = np.nan
...
def fluxo_caixa_fundo(ds, cnpj_fundo):
    ...
    df_dy *= ds.sel(CNPJ_Fundo=cnpj_fundo)['Patrimonio_Liquido'].shift(
        Data_Referencia=-1
    )
    ...
    df_amor = -ds.sel(CNPJ_Fundo=cnpj_fundo)[
        'Percentual_Amortizacao_Cotas_Mes'
    ]
    df_amor *= ds.sel(CNPJ_Fundo=cnpj_fundo)['Patrimonio_Liquido'].shift(
        Data_Referencia=-1
    )
    df_amor[df_amor == 0] = np.nan
    df_amor = df_amor.to_dataframe().drop(columns=['CNPJ_Fundo']).dropna()
    df_amor = df_amor.rename(
        columns={'Percentual_Amortizacao_Cotas_Mes': 'FC'}
    )
    ...
    df_fc = pd.concat(
        [df_fc_di, df_dy, df_amor, df_emiss], axis=0
